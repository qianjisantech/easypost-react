...
export function ParamsRunTab(props: ParamsTabProps) {
  ...
  const tabStatus = useMemo(() => {
    ...
    const hasPayload =
      Object.keys(payload).length > 0 && (payload.jsonSchema || payload.parameters || payload.type)
    ...
    () => [
      {
        key: 'params',
        label: (
          <BadgeLabel count={tabStatus.paramsCount} hasValue={tabStatus.paramsCount > 0}>
            Params
          </BadgeLabel>
        ),
        children: (
      ...
      {
        key: 'authorization',
        label: <BadgeLabel hasValue={tabStatus.hasAuthorization}>Authorization</BadgeLabel>,
        children: (
          <Authorization
      ...
      {
        key: 'body',
        label: <BadgeLabel hasValue={tabStatus.hasPayload}>Body</BadgeLabel>,
        children: (
          <ParamsPayload
            value={value?.payload}
            onChange={(payload) => {
      ...
      {
        key: 'headers',
        label: <BadgeLabel hasValue={tabStatus.hasHeaders}>Headers</BadgeLabel>,
        children: (
          <div className="pt-2">
            <ParamsEditableRunTable
              value={value?.header}
      ...
      {
        key: 'cookie',
        label: <BadgeLabel hasValue={tabStatus.hasCookies}>Cookie</BadgeLabel>,
        children: (
          <div className="pt-2">
            <ParamsEditableRunTable
              value={value?.cookie}
